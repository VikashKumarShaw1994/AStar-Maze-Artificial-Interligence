...
def make_maze():
    ...
    for i in range(5):
        ...
        for j in range(5):
            ...
            agent_maze[i].append("_")
            #actual maze
            if (j == 2 and i >= 1 and i <=3) or (j == 3 and i >= 2 and i <= 4):
                maze[i].append("X")
            else:
                maze[i].append("_")
    return maze, agent_maze
...
def astar(maze_start, maze_target, is_smaller_g):
    ...
    while start_node != maze_target:
        ...
        while path_cost[maze_target] > final_cost[min_node]:
            ...
            '''TEST HEAP
            if expanded_cnt % 2 == 0:
                heap_tuple = heapq.heappop(open_list_heap)
                heapq.heappush(open_list_heap , heap_tuple)
                heap_tuple2 = heap_pop(open_list_heap)
                if heap_tuple2 != heap_tuple:
                    print("******************HEAP ISSUE*****************")
                    print("Python heap pop: ",heap_tuple)
                    print("My heap pop: ",heap_tuple2)
                    return False
            else:
                heap_tuple2 = heap_pop(open_list_heap)
                heapq.heappush(open_list_heap, heap_tuple2)
                heap_tuple = heapq.heappop(open_list_heap)
                if heap_tuple2 != heap_tuple:
                    print("******************HEAP ISSUE*****************")
                    print("Python heap pop: ",heap_tuple)
